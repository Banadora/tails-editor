...
bool xEventFilter::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (    w->ui->nameEdit->hasFocus() || w->ui->nameNorth->hasFocus() ||
            w->ui->nameEast->hasFocus() || w->ui->nameSouth->hasFocus() || w->ui->nameWest->hasFocus())
    { return false; } //don't handle key event if editing text
